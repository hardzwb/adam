...
`NOC_ARB` 实现两路 NOC 端口到一路 SLLC 端口之间的 CHI 通道汇聚与分发。顶层端口包括三组 `CHI_NOC_ARB_IF` interface：

- `noc0`：NOC0 侧 CHI interface，使用 `noc_side` modport。
- `noc1`：NOC1 侧 CHI interface，使用 `noc_side` modport。
- `sllc`：SLLC 侧 CHI interface，使用 `sllc_side` modport。
...
1. 一个 `CHI_NOC_ARB_IF` interface 定义，封装链路控制信号和六个 CHI 通道信号。
2. 三个 interface 端口：`noc0`、`noc1`、`sllc`。
3. 三个 `SKY_LINK_CTRL_ACTIVE` 实例，分别封装 `noc0`、`noc1`、`sllc` 的建链/断链控制。
4. 六个 `SKY_CHI_CHANNEL_ARB` 实例，分别处理一个 CHI 通道的数据缓冲、credit 隔离和公平仲裁。
...
interface 内部的通道信号不再使用 `0/1` 后缀区分端口，而是通过 interface 实例名区分。例如：

- 原 `txreqflitpend0` 对应 `noc0.txreqflitpend`。
- 原 `txreqflitpend1` 对应 `noc1.txreqflitpend`。
- 原无后缀 `txreqflitpend` 对应 `sllc.txreqflitpend`。
